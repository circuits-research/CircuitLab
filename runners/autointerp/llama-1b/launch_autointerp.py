...
def build_config() -> AutoInterpConfig:
    ...
    return AutoInterpConfig(
        ...
        dtype="bfloat16",
        model_name=MODEL,
        clt_path=str(STORAGE_ROOT / "checkpoints" / f"{MODEL.replace('/', '_')}_BF16" / "7zue2yj0/final_307200000"),
        latent_cache_path=str(STORAGE_ROOT / "autointerp" / f"{MODEL.replace('/', '_')}"),
        dataset_path="chanind/openwebtext-llama3",
        context_size=16,
        total_autointerp_tokens=64 * 16 * 4096,
