...
class ActivationsStore:
    ...
    * Streams activations by: 
    ...
    def _load_buffer_from_cached(self, return_tokens:bool = False) -> tuple[torch.Tensor, ...]:
        ...
        if dist.is_initialized() and self.world_size > 1 and not (self.cfg.ddp or self.cfg.fsdp):
            cache_ptr_tensor = torch.tensor(self.cache_ptr, dtype=torch.int64, device=self.device)
            dist.broadcast(cache_ptr_tensor, src=0)
            self.cache_ptr = cache_ptr_tensor.item()
