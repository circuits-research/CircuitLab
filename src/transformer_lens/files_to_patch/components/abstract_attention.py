...
class AbstractAttention(ABC, nn.Module):
    ...
    def __init__(
        ...
        if self.cfg.use_sparse_attn:
            self.hook_sparse_mask = HookPoint()  # [batch, head_index, query_pos, key_pos]
    ...
    def forward(
        ...
        if self.cfg.use_sparse_attn:
            # only implemented for evaluation, not suitable for training or finetuning sparse attention
            sparse_mask = (attn_scores > 0).float()
            sparse_mask = self.hook_sparse_mask(sparse_mask)  # [batch, head_index, query_pos, key_pos]
            pattern = pattern * sparse_mask
